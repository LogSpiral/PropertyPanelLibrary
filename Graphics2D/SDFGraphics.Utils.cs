...
public partial class SDFGraphics
{
    private const float root2Over2 = 1.414213562373f / 2f;
    private static Effect SDF_Effect => ModAsset.SDFGraphics.Value;
    private static bool DrawFrame => false;
    ...
    private static void SetBackground(Color background) => SDF_Effect.Parameters["uBackgroundColor"].SetValue(background.ToVector4());

    private static void SetBorder(float border, Color borderColor)
    {
    ...
    private static void SetCommon(Matrix matrix)
    {
        Effect effect = SDF_Effect;
    ...
    private static void SetBar(Texture2D barTexture, float time, float distanceScaler)
    {
        Effect effect = SDF_Effect;
    ...
    private static void ApplyPass(string passName) => SDF_Effect.CurrentTechnique.Passes[passName].Apply();
    ...
    private static void DrawLine(Vector2 start, Vector2 end, float LineWidth)
    {
    ...
    {
        return Vector2.DistanceSquared(Main.MouseScreen, roundCenter) <= radius * radius;
    }
